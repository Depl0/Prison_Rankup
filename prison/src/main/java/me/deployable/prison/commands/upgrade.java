...
public class upgrade implements CommandExecutor {
    ...
    private boolean takeEconomy(Player player, int cost, String group) {
        // Execute the "eco take" command
        String playerName = player.getName();
        String command = "eco take " + playerName + " " + cost + " " + group;
        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);

        // Check if the economy operation was successful
        // You may need to implement additional logic to verify the result

        return true; // Return true for success (update as needed)
    }
}
